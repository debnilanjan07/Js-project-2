...
let modal_box = document.querySelector(".smallbox");
...
function isEmptyOrNot() {
  if (flexContainer.innerHTML == "") {
    document.getElementById("no-item-div").classList.remove("hide");
  } else {
    document.getElementById("no-item-div").classList.add("hide");
  }
}
    ...
    boxes.forEach((user) => {
      if (user.id == id) {
        ...
        <div class='sbox'>
          ...
          <i id="s_delete_list" onclick="delBox(${user.id})" class="fas fa-trash"></i>
